...
const allIssues: (Issue & { project: string })[] = Object.entries(issuesByProject).flatMap(
  ([project, issues]) => issues.map((issue) => ({ ...issue, project })),
);
 ...
 * @param {string} issueId - The issue ID (e.g. "ALPHA-1").
 * @returns {Issue | null} The issue, or `null` if not found.
 */
