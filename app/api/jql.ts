...
import type { Issue } from "~/api/jira-api";
...
function parseCondition(
  tokens: string[],
  pos: number,
): { expr: Expression; pos: number } {
  if (pos + 2 >= tokens.length) {
    throw new Error(
      `Expected condition at position ${pos}, but not enough tokens`,
    );
  }
...
function evaluateExpression(
  ...
  switch (expr.type) {
    ...
    case "and":
      return (
        evaluateExpression(issue, expr.left) &&
        evaluateExpression(issue, expr.right)
      );
    case "or":
      return (
        evaluateExpression(issue, expr.left) ||
